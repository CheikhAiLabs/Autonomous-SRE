...
.PHONY: help init configure check fmt test plan deploy deploy-all deploy-ci verify status logs headlamp chaos-smoke chaos-replica-floor chaos-bad-release access destroy destroy-all
...
help:
	...
	@echo "  make headlamp             Open protected Headlamp in a new browser tab"
